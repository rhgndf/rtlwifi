						 ...
						 enum radio_path rfpath)
{
	...
	if (rtlhal->ic_class != IC_INFERIORITY_A) {
		tmpval = rtl_get_rfreg(hw, rfpath, RF_IPA, 0xf);
		rtl_set_rfreg(hw, rfpath, RF_IPA, 0xf, tmpval + 1);
	}
...
static bool _rtl92s_phy_config_bb(struct ieee80211_hw *hw, u8 configtype)
{
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		for (i = 0; i < phy_reg_len; i = i + 2) {
			...
			rtl_set_bbreg(hw, phy_reg_table[i], MASKDWORD,
					phy_reg_table[i + 1]);
		}
	} else if (configtype == BASEBAND_CONFIG_AGC_TAB) {
		for (i = 0; i < agc_len; i = i + 2) {
			rtl_set_bbreg(hw, agc_table[i], MASKDWORD,
					agc_table[i + 1]);
					  ...
					  u8 configtype)
{
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		for (i = 0; i < phy_regarray2xtxr_len; i = i + 3) {
			...
			rtl_set_bbreg(hw, phy_regarray2xtxr_table[i],
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		for (i = 0; i < phy_pg_len; i = i + 3) {
			...
			_rtl92s_store_pwrindex_diffrate_offset(hw,
					...
					phy_table_pg[i + 1],
					phy_table_pg[i + 2]);
			rtl_set_bbreg(hw, phy_table_pg[i],
					phy_table_pg[i + 1],
					phy_table_pg[i + 2]);
		}
	}
...
bool rtl92s_phy_bb_config(struct ieee80211_hw *hw)
{
	...
	path1 = (u8)(rtl_get_bbreg(hw, RFPGA0_TXINFO, 0xf));
	mdelay(10);
	path2 = (u8)(rtl_get_bbreg(hw, ROFDM0_TRXPATHENABLE, 0xf));
	pathmap = path1 | path2;
