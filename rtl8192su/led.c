...
static void _rtl92su_init_led(struct ieee80211_hw *hw,
			      enum rtl_led_pin *pled, enum rtl_led_pin ledpin)
{
	*pled = ledpin;
}

void rtl92su_sw_led_on(struct ieee80211_hw *hw, enum rtl_led_pin ledpin)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u8 ledcfg = rtl_read_byte(rtlpriv, LEDCFG);
...
void rtl92su_sw_led_off(struct ieee80211_hw *hw, enum rtl_led_pin ledpin)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	u8 ledcfg = rtl_read_byte(rtlpriv, LEDCFG);
...
static void _rtl92su_deInit_led(enum rtl_led_pin *pled)
{
}
